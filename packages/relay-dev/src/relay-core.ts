...
// ── Minimal socket abstraction (a real `ws` WebSocket satisfies this) ─────────
export interface RelayWS {
  send(data: string): void;
  close(code?: number): void;
  readonly id: number; // stable identity for last-writer-wins comparisons
}

export type VerifyFn = (
  envelope: { payloadStr?: string; signature?: string },
  publicKeyHex: string,
...
export interface RelayCoreDeps {
  ...
  now: () => number;          // Date.now in prod; fixed in tests
  randomId: () => string;     // uuid in prod; deterministic in tests
  randomNonce: () => string;  // 32-byte hex in prod
  log?: (msg: string) => void;
}
...
interface Conn {
  socket: RelayWS;
  pubkey?: string;            // set after auth-ok (receivers registered in routing)
...
export class RelayCore {
  ...
  readonly routing = new Map<string, RelayWS>();
  private readonly challenges = new Map<string, PendingChallenge>();
  private readonly conns = new Map<number, Conn>();

  constructor(private readonly deps: RelayCoreDeps) {}
  ...
  onMessage(socket: RelayWS, raw: string): void {
    ...
    let frame: { type?: unknown; [k: string]: unknown };
    try {
  ...
  onClose(socket: RelayWS): void {
    ...
    if (conn?.challengeId) this.challenges.delete(conn.challengeId);
    this.conns.delete(socket.id);
  }

  // ── handlers ────────────────────────────────────────────────────────────────

  private handleAuth(socket: RelayWS, conn: Conn | undefined, frame: Record<string, unknown>): void {
    ...
    if (conn) { conn.pubkey = pubkey; conn.role = role; }

    if (role === 'receiver') {
      // last-writer-wins: drop any prior socket for this pubkey
      const prior = this.routing.get(pubkey);
      if (prior && prior !== socket) { try { prior.close(1000); } catch { /* ignore */ } }
  ...
  verifyAuthFrame(
  ...
  ): { ok: true; pubkey: string; role: 'receiver' | 'sender'; challengeId: string }
    ...
    let parsed: { pubkey?: unknown; challengeId?: unknown; nonce?: unknown; role?: unknown };
    try {
    ...
    if (!v.ok) return { ok: false, reason: v.reason ?? 'bad-signature' };
    return { ok: true, pubkey, role, challengeId };
  }

  private handleDeliver(socket: RelayWS, conn: Conn | undefined, frame: Record<string, unknown>): void {
    if (!conn?.pubkey) {
   ...
   */
  private handleFederation(socket: RelayWS, conn: Conn | undefined, frame: Record<string, unknown>): void {
    if (!conn?.pubkey) {
      socket.send(JSON.stringify({ type: 'error', reqId: frame.reqId, code: 'unauthorized', message: 'auth required' }));
      return;
    }
  ...
  private readonly responseRoute = new Map<string, { replyTo: RelayWS; expiresAt: number }>();

  private handleResponse(conn: Conn | undefined, frame: Record<string, unknown>): void {
    if (!conn?.pubkey) return; // only authed receivers answer
    const reqId = frame.reqId;
    if (typeof reqId !== 'string') return;
    const route = this.responseRoute.get(reqId);
    this.responseRoute.delete(reqId);
    if (!route) return; // sender gone / expired — drop
  ...
  private sweepChallenges(): void {
    ...
    for (const [id, c] of this.challenges) {
      if (c.createdAt < cutoff) this.challenges.delete(id);
    }
    // also expire stale response routes
    const now = this.deps.now();
    for (const [id, r] of this.responseRoute) {
      if (r.expiresAt < now) this.responseRoute.delete(id);
    }
  }
}
