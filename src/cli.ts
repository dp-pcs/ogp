  ...
  if (forFlag) {
    ...
    if (!metaConfig.frameworks || metaConfig.frameworks.length === 0) {
      if (process.env.OGP_HOME) {
        console.error(
          `Warning: --for ${forFlag} given but no frameworks are configured; ` +
...
federation
  ...
  .action(async (peerUrl, peerId, options) => {
    ...
    if (options.petname) {
      if (!alias) {
        if (!json) console.warn('⚠️  --petname is deprecated. Use --alias instead.');
        alias = options.petname;
      } else {
        if (!json) console.warn('⚠️  Both --alias and --petname provided. Using --alias.');
      }
    }
    ...
    if (!peerId) {
      try {
        ...
        if (!json) console.log(`Resolving peer ID from ${wellKnownUrl}...`);
        ...
        if (!json) console.log(`✓ Resolved peer: ${data.displayName || 'Unknown'} (${peerId.slice(0, 16)}...)`);
      } catch (err: any) {
        if (json) {
