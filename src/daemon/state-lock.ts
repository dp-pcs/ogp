...
export class StateDirLockedError extends Error {
  ...
  constructor(stateDir: string, lockPath: string, holderPid: number | null) {
    ...
    super(
      `Another OGP daemon (${who}) is already running on stateDir ${stateDir}. ` +
 ...
 */
export function acquireStateDirLock(
  stateDir: string,
  opts: AcquireStateDirLockOptions = {}
): StateDirLockHandle {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      try { fs.unlinkSync(lockPath); } catch { /* lost a race; retry loop handles it */ }
      continue;
    }
