...
function signalLockHandshake(ok: boolean, reason?: string): void {
  if (lockHandshakeSignalled) return;
  if (process.env.OGP_LOCK_HANDSHAKE !== '1') return;
  ...
  try { fs.closeSync(HANDSHAKE_FD); } catch { /* best effort */ }
}
...
interface ApprovalValidationDeps {
  verifyEnvelope: (
    env: { payloadStr?: string; signature?: string },
    publicKey: string
...
export function verifySignedPeerIdHeader(
  headers: SignedPeerIdHeaders,
  publicKey: string,
  verifyImpl: (msg: string, sig: string, pk: string) => boolean,
  opts: { now?: number; maxAgeMs?: number } = {}
): boolean {
  const { peerId, timestamp, signature } = headers;
  if (!peerId || !timestamp || !signature || !publicKey) return false;
...
export function validateSignedApproval(
  body: any,
...
export function startServer(config?: OGPConfig, background = false): void {
  ...
  if (background) {
    ...
    const finishParent = (ok: boolean, holderMessage?: string): void => {
      ...
      if (ok) {
        ...
        if (holderMessage) console.error(`✗ ${holderMessage}`);
        else console.error('✗ OGP daemon failed to acquire the stateDir lock (see daemon log).');
        process.exit(1);
      }
    };

    // Read the handshake. A single OGP_LOCK_OK line => the child holds the lock.
    const handshake = child.stdio[3] as NodeJS.ReadableStream | null;
    let handshakeBuf = '';
    let handshakeFailMsg: string | undefined;
    if (handshake) {
      handshake.setEncoding('utf-8');
      handshake.on('data', (chunk: string) => {
        handshakeBuf += chunk;
        if (handshakeBuf.includes('OGP_LOCK_OK')) { finishParent(true); return; }
  ...
  app.get('/.well-known/ogp', async (req: Request, res: Response) => {
  ...
  app.post('/federation/reply/:nonce', async (req: Request, res: Response) => {
    const { nonce } = req.params;
    const body = req.body || {};
    ...
    let signingPeer: Peer | null = null;
