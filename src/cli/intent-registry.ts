...
import type { Intent } from '../daemon/intent-registry.js';
...
export function registerNewIntent(name: string, options: RegisterOptions): void {
  // Validation
  if (!name || name.trim() === '') {
...
export function listRegisteredIntents(): void {
...
export function removeIntent(name: string): void {
  if (!name || name.trim() === '') {
    console.error('Error: Intent name is required');
    process.exit(1);
  }
