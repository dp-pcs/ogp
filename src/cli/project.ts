 ...
 */
export function buildPeerQueryJson(
  projectId: string,
  contributions: any[]
): PeerQueryContributionJson[] {
  return (contributions ?? []).map((c) => {
    const entryType = c.entryType ?? c.topic;
    const out: PeerQueryContributionJson = {
      id: c.id,
      projectId,
      authorId: c.authorId,
      entryType,
...
export async function projectCreate(
  ...
  if (!project.members.includes(myKey)) project.members.push(myKey);
...
export async function projectQuery(
  ...
  if (options.search) {
    ...
    if (!options.json) console.log(`Search results for "${options.search}" in project '${project.name}':`);
  } else if (entryType) {
    // Query by entry type
    contributions = getTopicContributions(projectId, entryType, limit);
    if (!options.json) console.log(`Contributions [${entryType}] in project '${project.name}':`);
  } else if (options.author) {
    // Query by author
    contributions = getAuthorContributions(projectId, options.author, limit);
    if (!options.json) console.log(`Contributions by '${options.author}' in project '${project.name}':`);
  } else {
...
export async function projectClaimOwnership(projectId: string): Promise<void> {
  ...
  const project = getProject(projectId);
  if (!project) { console.error(`Error: Project '${projectId}' not found`); process.exit(1); }
  if (project.creation?.provenance === 'original') { console.error('Error: Project already has an original owner'); process.exit(1); }
  ...
  if (!member) { console.error('Error: Only an existing project member may claim ownership'); process.exit(1); }
  ...
  if (result === 'rejected' || result === 'exists-original') { console.error(`Error: claim failed (${result})`); process.exit(1); }
  const { listPeers } = await import('../daemon/peers.js');
  const peers = listPeers('approved').filter(p => listProjectsForPeer(p.id, [project]).length > 0);
  for (const peer of peers) {
    try { await federationSend(peer.id, 'project.create', JSON.stringify({ projectId, projectName: project.name, creation }), 30000); } catch { /* best-effort */ }
  }
