...
function getHumanSurfacingDefault(mode: InboundFederationMode): HumanSurfacingMode {
  if (mode === 'forward') {
    return 'always';
  }
...
function getRelayHandlingDefault(mode: InboundFederationMode): RelayHandlingMode {
  if (mode === 'approval-required') {
...
async function promptInboundFederationMode(
  rl: readline.Interface,
  currentMode: InboundFederationMode = 'summarize'
): Promise<InboundFederationMode> {
...
async function promptHumanSurfacingMode(
  rl: readline.Interface,
  defaultMode: InboundFederationMode,
  currentMode: HumanSurfacingMode = getHumanSurfacingDefault(defaultMode)
): Promise<HumanSurfacingMode> {
...
async function promptRelayHandlingMode(
  rl: readline.Interface,
  defaultMode: InboundFederationMode,
  currentMode: RelayHandlingMode = getRelayHandlingDefault(defaultMode)
): Promise<RelayHandlingMode> {
...
async function promptApprovalTopics(
  rl: readline.Interface,
...
async function promptTrustedPeerAutonomy(
  rl: readline.Interface,
  currentValue = true
): Promise<boolean> {
  return promptYesNo(
    rl,
    'Should trusted peers be eligible for more autonomy than the default policy later?',
    currentValue
  );
}
...
export function buildDelegatedAuthorityConfig(options: {
  ...
  const topicRules = approvalTopics.reduce<Record<string, NonNullable<DelegatedAuthorityConfig['global']['topicRules']>[string]>>(
    (rules, topic) => {
      rules[topic] = {
...
export function deriveDelegatedAuthorityInterviewAnswers(config: OGPConfig): DelegatedAuthorityInterviewAnswers {
  const inboundFederationMode =
    config.delegatedAuthority?.global.defaultRule.mode ??
    config.inboundFederationPolicy?.mode ??
    'summarize';

  const humanSurfacingMode =
    config.delegatedAuthority?.global.defaultRule.surfaceToHuman ??
    getHumanSurfacingDefault(inboundFederationMode);

  const relayHandlingMode =
    config.delegatedAuthority?.global.classRules?.['human-relay']?.relayMode ??
...
export function applyDelegatedAuthorityInterviewAnswers(
  config: OGPConfig,
  answers: DelegatedAuthorityInterviewAnswers
): OGPConfig {
  return {
    ...config,
...
async function promptMultiSelect(
  ...
  const selected: DetectedFramework[] = [];
...
export async function runAgentCommsInterview(): Promise<void> {
  ...
  try {
    ...
    const humanSurfacingMode = await promptHumanSurfacingMode(
      rl,
      inboundFederationMode,
      defaults.humanSurfacingMode
    );
    const relayHandlingMode = await promptRelayHandlingMode(
      rl,
      inboundFederationMode,
      defaults.relayHandlingMode
    );
