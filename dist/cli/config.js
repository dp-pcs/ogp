...
import { loadConfig, saveConfig, requireConfig, synthesizePersonas, effectiveHookAgentId, resolveTransportList } from '../shared/config.js';
import { loadHealthCheckConfig, getHeartbeatConfig } from '../daemon/heartbeat.js';
function printPersonas(config) {
    ...
    personas.forEach((persona) => {
        ...
        console.log(`  ${marker} ${persona.id.padEnd(12)} ${persona.displayName.padEnd(16)} ${persona.role.padEnd(10)} -> hookAgentId: ${effectiveHookAgentId(persona)}`);
    });
    console.log('');
}
/**
 ...
 */
export function whoami(json = false) {
    const config = requireConfig();
    const meta = loadMetaConfig();
    ...
    if (config.humanName || config.agentName) {
        const parts = [];
        if (config.humanName)
            parts.push(config.humanName);
        if (config.agentName)
            parts.push(`(${config.agentName})`);
 ...
 */
export function listAgents(quiet = false) {
    const config = requireConfig();
    const personas = synthesizePersonas(config);
    if (quiet) {
 ...
 */
const VALID_TRANSPORT_MODES = ['direct', 'relay', 'iroh'];
function showTransport(opts = {}) {
    const config = requireConfig();
    ...
    if (t?.relay?.url)
        console.log(`  relay url: ${t.relay.url}`);
    if (t?.iroh?.relayUrl)
        console.log(`  iroh relay url: ${t.iroh.relayUrl}`);
...
function setTransportMode(mode) {
    if (!VALID_TRANSPORT_MODES.includes(mode)) {
        console.error(`Error: invalid transport mode '${mode}'. Valid: ${VALID_TRANSPORT_MODES.join(', ')}`);
        process.exit(1);
    }
    const config = requireConfig();
    config.transport = { ...(config.transport ?? {}), mode: mode };
    saveConfig(config);
...
function setTransportRelayUrl(url) {
    const config = requireConfig();
    const existing = config.transport ?? { mode: 'relay' };
 ...
 */
function setTransportAdvertise(modes) {
    const invalid = modes.filter((m) => !VALID_TRANSPORT_MODES.includes(m));
    if (invalid.length > 0) {
    ...
    const config = requireConfig();
    config.transport = { ...(config.transport ?? { mode: 'direct' }), advertise: modes };
    saveConfig(config);
...
function setTransportPrefer(mode) {
    if (!VALID_TRANSPORT_MODES.includes(mode)) {
        console.error(`Error: invalid transport mode '${mode}'. Valid: ${VALID_TRANSPORT_MODES.join(', ')}`);
        process.exit(1);
    }
    const config = requireConfig();
    config.transport = { ...(config.transport ?? { mode: 'direct' }), prefer: mode };
    saveConfig(config);
    console.log(`✓ Preferred transport set to: ${mode}`);
    const advertise = config.transport?.advertise;
    if (advertise && advertise.length > 0 && !advertise.includes(mode)) {
