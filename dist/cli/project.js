...
import { backfillContributionsFromPeer } from '../daemon/contribution-backfill.js';
 ...
 */
export function liveBackfillDeps() {
    return {
 ...
 */
export async function backfillProjectFromMembers(projectId, selfId, deps = liveBackfillDeps(), opts = {}) {
 ...
 */
function formatAuthorDisplay(contribution) {
...
export async function projectJoin(projectId, projectName, options = {}) {
    ...
    try {
        ...
        if (results.length > 0)
            console.log(`  ${summarizeBackfill(results)}`);
    }
    catch (err) {
 ...
 */
export async function projectContribute(projectId, entryType, summary, options = {}) {
    const config = loadConfig();
    if (!config) {
        console.error('Error: Not configured. Run "ogp setup" first.');
        process.exit(1);
    ...
    try {
        ...
        if (response.success && response.response?.joined) {
            ...
            try {
                ...
                if (all.some(r => r.pulled > 0 || r.error))
                    console.log(`  ${summary}`);
            }
            catch (err) {
                console.warn(`  backfill skipped: ${err.message}`);
 ...
 */
export async function projectSendContribution(peerId, projectId, entryType, summary, options = {}) {
    const config = loadConfig();
    if (!config) {
        console.error('Error: Not configured. Run "ogp setup" first.');
        process.exit(1);
    ...
    try {
        const result = await federationSend(peerId, 'project.contribute', JSON.stringify(payload), undefined, options.toAgent);
        if (result === null) {
...
export async function projectQueryPeer(peerId, projectId, options = {}) {
    ...
    try {
        ...
        if (contributions && contributions.length > 0) {
            ...
            const filters = [];
            if (entryType)
                filters.push(`entry type: ${entryType}`);
            if (options.author)
