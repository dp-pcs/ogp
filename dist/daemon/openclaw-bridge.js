...
function buildGatewayWsUrls(url) {
...
function buildHookBaseUrls(url) {
    const base = normalizeBaseUrl(url);
...
async function callGatewayMethod(params) {
    ...
    for (const candidate of candidates) {
        try {
            ...
            console.error(`[OGP Bridge] ${params.method} returned unexpected output via ${candidate}:`, stdout.trim() || stderr.trim());
        }
        catch (err) {
