...
import { acquireStateDirLock, StateDirLockedError } from './state-lock.js';
let server = null;
 ...
 */
export async function handleFederationRequestCore(body, deps) {
 ...
 */
export async function handleFederationApproveCore(body, deps) {
    const { cfg, verifyEnvelope } = deps;
    try {
        const safeBody = (body || {});
            ...
            try {
                ...
                if (cfg.rendezvous?.enabled && freshPeer.publicKey && !freshPeer.gatewayUrl) {
                    try {
                        ...
                        if (relay && relay.mode === 'relay')
                            relayBack = relay.relayUrl;
                    }
